...
class GlobalContext:
    ...
    POSTGRES_DB_USERNAME: Union[str, None] = \
        get_environment_variable('POSTGRES_DB_USERNAME')
    POSTGRES_DB_PASSWORD: Union[str, None] = \
        get_environment_variable('POSTGRES_DB_PASSWORD')
    POSTGRES_DB_HOST: Union[str, None] = \
        get_environment_variable('POSTGRES_DB_HOST')
    POSTGRES_DB_PORT: Union[str, None] = \
        get_environment_variable('POSTGRES_DB_PORT')

    INITIAL_DATE: date = \
        get_environment_variable(
            'SIMULATOR_INITIAL_DATE',
            date,
            date.today()
        )
    ...
    CURRENCY_MULTIPLIER: float = \
        get_environment_variable(
            'SIMULATOR_CURRENCY_MULTIPLIER',
            float,
            1.0
        )

    POPULATION_FAMILY_SIZE: float = \
        get_environment_variable(
            'SIMULATOR_POPULATION_FAMILY_SIZE',
            float,
            3.0
        )
    POPULATION_FERTILITY_RATE: float = \
        get_environment_variable(
            'SIMULATOR_POPULATION_FERTILITY_RATE',
            float,
            0.1
        )
    POPULATION_LIFE_EXPECTANCY: float = \
        get_environment_variable(
            'SIMULATOR_POPULATION_LIFE_EXPECTANCY',
            float,
            71.0
        )
    POPULATION_MARRY_AGE: float = \
        get_environment_variable(
    ...
    POPULATION_PURCHASING_POWER: float = \
        get_environment_variable(
            'SIMULATOR_POPULATION_PURCHASING_POWER',
            float,
            3400.0
        )
    POPULATION_SPENDING_RATE: float = \
        get_environment_variable(
    ...
    INITIAL_STORES: int = \
        get_environment_variable('SIMULATOR_INITIAL_STORES', int, 1)
    INITIAL_STORES_RANGE_DAYS: int = \
        get_environment_variable('SIMULATOR_INITIAL_STORES_RANGE_DAYS', int, 0)
    STORE_MARKET_POPULATION: int = \
        get_environment_variable(
            'SIMULATOR_STORE_MARKET_POPULATION',
            int,
            10_000
        )
    STORE_GROWTH_RATE: int = \
        get_environment_variable('SIMULATOR_STORE_GROWTH_RATE', float, 0.5)
    ...
    CONFIG_ITEM_PATH: Path = \
        get_environment_variable(
            'SIMULATOR_CONFIG_ITEM_PATH',
            Path,
            CONFIG_DIR / 'items.yaml'
        )
    CONFIG_ITEM = None

    CONFIG_LOCATION_PATH: Path = \
        get_environment_variable(
            'SIMULATOR_CONFIG_LOCATION_PATH',
            Path,
    ...
    DEBUG_DATABASE: bool = \
        get_environment_variable('DEBUG_DATABASE', bool, False)
    DEBUG_SIMULATOR: bool = \
        get_environment_variable('DEBUG_SIMULATOR', bool, False)
    ...
    def get_config_item(cls, config_path: Path = None) -> dict:
        if config_path is None:
            config_path = cls.CONFIG_ITEM_PATH
    ...
    def get_config_location(cls, config_path: Path = None) -> dict:
        if config_path is None:
            config_path = cls.CONFIG_LOCATION_PATH
